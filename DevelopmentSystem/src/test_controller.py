...
class TestController:
    ...
    def run(self, operational_mode: str, dataset: dict = None, test_error_threshold: float = None):
        if operational_mode == 'test_best_classifier':
            ...
            errors = {
                'training_error': self.mental_command_classifier.get_error(dataset['training_data'],
                                                                           dataset['training_labels']),
                'test_error': self.mental_command_classifier.get_error(dataset['test_data'], dataset['test_labels'])
            }

            # prepare parameters needed for the report
            training_parameters = {
                'number_of_generations':  self.mental_command_classifier.get_number_of_generations(),
                'hidden_layer_sizes': self.mental_command_classifier.get_hidden_layer_sizes()
            }
